    ...
    int nsamples)
{
     ...
     */
    static_assert(FBFUSE_NBEAMS%FBFUSE_CB_WARP_SIZE==0,
     ...
     */
    int const fbpa_weights_offset = FBFUSE_CB_NANTENNAS/4 * (FBFUSE_NBEAMS * blockIdx.y + (FBFUSE_CB_WARP_SIZE * blockIdx.z + warp_idx));
    ...
    {
        int ftpa_voltages_partial_idx = FBFUSE_CB_NANTENNAS/4 * NPOL * (nsamples * blockIdx.y + sample_idx);
     ...
     * as the total volume of data being written out is a factor of NACCUMULATE * FBFUSE_CB_NANTENNAS / FBFUSE_CB_WARP_SIZE
     * smaller than the input (e.g. for 64 antennas and 16 integrated samples this is a factor of 32).
     */

    /** ORIGINAL
    int output_idx = (NWARPS_PER_BLOCK * gridDim.x) * (FBFUSE_NBEAMS * blockIdx.y
