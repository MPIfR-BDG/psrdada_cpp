...
// In the output transpose we go from FBT to BTF

// Need to read N of the inner dimension:
//  - 32 time samples for each frequency
//  - transpose in shared memory
//  - write out 32 frequency channels
//  - should operate in tiles of some minimum
//    dimension.
//
//  Can we put this transpose into the beamformer along
//  with a compression back to 8-bits.


__global__
void input_transpose_k(
    ...
    int ntimestamps)
{
    ...
    for (int timestamp_idx = blockIdx.x; timestamp_idx < ntimestamps; timestamp_idx += gridDim.x)
    {
        for (int chan_idx = blockIdx.y; chan_idx < nchans; chan_idx += gridDim.y)
        {
            ...
            {
                int remaining_antennas = min(nantennas - antenna_set_idx * MAX_ANTENNAS, MAX_ANTENNAS);
                // Load data into shared memory
                for (int antenna_idx = threadIdx.y; antenna_idx < remaining_antennas; antenna_idx += blockDim.y)
                {
                    int input_antenna_idx = antenna_set_idx * MAX_ANTENNAS + antenna_idx;
                ...
                for (int pol_idx = 0; pol_idx < NPOL; ++pol_idx)
                {
                    for (int samp_idx = threadIdx.y; samp_idx < NSAMPS_PER_TIMESTAMP; samp_idx += blockDim.y)
                    {
                        ...
                        for (int antenna_idx = threadIdx.x; antenna_idx < remaining_antennas; antenna_idx += blockDim.x)
                        {
                            ...
                            //FTPA
                            int output_idx = chan_idx * tpa + output_sample_idx * pa + pol_idx * nantennas + output_antenna_idx;
    ...
    int ntimestamps)
{
    ...
    int ftp = nchans * tp;
    int aftp = nantennas * ftp;

    int pa = NPOL * nantennas;
...
int main()
{
    ...
    std::size_t size = ntimestamps * nantennas * nchans * NSAMPS_PER_TIMESTAMP * NPOL;
    ...
    thrust::host_vector<char2> input_h(size, default_value);
    thrust::host_vector<char2> output_h(size, default_value);

    populate<char2>(input_h.data(), size, 0, 64);
    ...
    */
    ...
    dump_host_buffer<char2>(thrust::raw_pointer_cast(input_h.data()), size, "input_data.bin");
    ...
    thrust::device_vector<char2> input(size);
    thrust::device_vector<char2> output(size);
    ...
    {
        input_transpose_simple_k<<< grid, block >>>( input_ptr, output_ptr, nantennas, nchans, ntimestamps);
    }
    ...
    thrust::host_vector<char2> cpu_output(size);
    CUDA_ERROR_CHECK(cudaDeviceSynchronize());
    c_reference(input_h.data(), cpu_output.data(), nantennas, nchans, ntimestamps);

    dump_host_buffer<char2>(thrust::raw_pointer_cast(gpu_output.data()), size, "gpu_output_data.bin");
    dump_host_buffer<char2>(thrust::raw_pointer_cast(cpu_output.data()), size, "cpu_output_data.bin");

if (!is_same(cpu_output.data(), gpu_output.data(), size))
        std::cout << "FAILED!\n";
    else
        std::cout << "PASSED!\n";
#endif
