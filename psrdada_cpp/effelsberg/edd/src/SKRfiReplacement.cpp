...
SKRfiReplacement::SKRfiReplacement(const std::vector<std::complex<float>> &data, const std::vector<int> &rfi_status)
    : _data(data),
...
void SKRfiReplacement::init()
{
    ...
    _window_size = _data.size() / _nwindows;
...
void SKRfiReplacement::get_clean_window_indices()
{
    ...
    _clean_window_indices.resize(n);
    std::size_t iter = 0;
...
void SKRfiReplacement::get_clean_data_statistics(DataStatistics &ref_data_statistics)
{
	...
	std::copy((_data.begin() + ibegin), (_data.begin() + iend), (clean_data.begin() + jj));
...
void SKRfiReplacement::generate_replacement_data(DataStatistics stats, std::vector<std::complex<float>> &replacement_data)
{
...
void SKRfiReplacement::replace_rfi_data(std::vector<std::complex<float>> &rfi_replaced_data)
{
    ...
    if((_nrfi_windows > 0) && (_nrfi_windows < _nwindows)){
        get_clean_data_statistics(stats);
	...
	for(std::size_t ii = 0; ii < _nrfi_windows; ii++){
	    ...
	    std::copy(replacement_data.begin(), replacement_data.end(), (rfi_replaced_data.begin() +index));
	}
    }
}
} //edd
