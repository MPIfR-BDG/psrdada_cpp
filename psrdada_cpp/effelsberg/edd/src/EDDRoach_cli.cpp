	...
	std::size_t nsamps_per_heap;
       ...
       ("output_key,o", po::value<std::string>()
        ...
        try
        {
            ...
            if ( vm.count("help")  )
            {
                std::cout << "EDDRoach -- Read EDD data from a DADA buffer and merge the polarizations"
         ...
         */
        MultiLog log("edd::EDDRoach");
	DadaWriteClient output(output_key, log);
	effelsberg::edd::EDDRoach mod(nsamps_per_heap, npol, output);
